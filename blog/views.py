...
def post_detail(request,year,month,day,post):
  ...
  similar_posts = Post.published_posts.filter(tags__in=post_tags_ids) \
                                .exclude(id=post.id)
...
def post_search(request):
  ...
  if 'query' in request.GET:
    ...
    if form.is_valid():
      ...
      # search_vector2 var is to add weight to query
      search_vector2 = SearchVector('title',weight='A') + \
                       SearchVector('body',weight="B")
      search_query = SearchQuery(query,config='spanish')

      results = Post.published_posts.annotate(
        search=search_vector,
        rank=SearchRank(search_vector2,search_query)
